...
const HELP_CARDS = [
  { title: "AI Companion",        desc: "Talk through how you're feeling",  bg: "#e8ede8", IconComp: MessageCircle, emoji: "\u2764\uFE0F", href: "/(tabs)/companion" },
  { title: "Personal Wellness",   desc: "Medicine & self-care reminders",   bg: "#f0e8dc", IconComp: Heart,         emoji: "\uD83E\uDE7A", href: "/(tabs)/wellness" },
  { title: "Family Coordination", desc: "Share the caregiving load",        bg: "#e4eae4", IconComp: Users,         emoji: "\uD83C\uDFE0", href: "/(tabs)/family" },
  { title: "Solace Time",         desc: "A few moments just for you",       bg: "#eae6d8", IconComp: Sparkles,      emoji: "\uD83C\uDF38", href: "/(tabs)/solace" },
];

export default function HomeScreen() {
  const router = useRouter();
  const { isMobile, r } = useBreakpoint();
  ...
  return (
    <View style={styles.page}>
      ...
      <View style={styles.heroCard}>
        {/* Only show logo row on mobile (top nav shows it on desktop) */}
        {isMobile && (
        ...
        <Text style={[styles.heroGreeting, isMobile && styles.heroGreetingMobile]}>
          {getGreeting()}..
        </Text>
          ...
          {reminders.map((r, i) => (
            <View key={r.id}>
              ...
              <View style={styles.reminderRow}>
                ...
                <Text style={[styles.reminderName, r.completed && styles.strikethrough]} numberOfLines={1}>
                  {r.name}
                </Text>
      ...
      <View style={styles.section}>
        ...
        <View style={[styles.helpGrid, isMobile && styles.helpGridMobile]}>
          {HELP_CARDS.map((h) => (
            <TouchableOpacity
              key={h.title}
              style={[styles.helpCard, isMobile && styles.helpCardMobile, { backgroundColor: h.bg }]}
...
const styles = StyleSheet.create({
  ...
  heroGreetingMobile: { fontSize: 28 },
  heroSubtitle: {
  ...
  heroPill: {
    ...
    borderRadius: Radii.pill, borderWidth: 1, borderColor: "#d8d4c8",
  },
  ...
  reminderRow: { flexDirection: "row", alignItems: "center", paddingVertical: 14, gap: 10 },
  circle: {
    width: 20, height: 20, borderRadius: 10,
    borderWidth: 1.5, borderColor: "#c8c4b8", flexShrink: 0,
  },
  circleChecked: { backgroundColor: Colors.primary, borderColor: Colors.primary },
  reminderCategory: {
    fontFamily: FontFamily.sans, fontSize: FontSize.sm,
    color: Colors.textMuted, width: 76,
  },
  reminderName: {
    fontFamily: FontFamily.sansMedium, fontSize: FontSize.sm,
  ...
  helpGridMobile: { gap: 10 },
  helpCard: {
    width: "47.5%",
    borderRadius: Radii.xl, padding: 18,
    borderWidth: 1, borderColor: "#e0ddd4", minHeight: 130,
  },
  helpCardMobile: { width: "47%", padding: 14, minHeight: 110 },
  helpCardTop: {
    flexDirection: "row", justifyContent: "space-between",
  ...
  helpCardDesc: {
    fontFamily: FontFamily.sans, fontSize: FontSize.xs,
    color: Colors.textSecondary, lineHeight: 16, flex: 1,
  },
  helpOpen: { fontFamily: FontFamily.sansMedium, fontSize: FontSize.xs, color: Colors.primary, marginTop: 8 },
});